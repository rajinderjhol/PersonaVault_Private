...
async def stream_chat(
    ...
    async def generate() -> AsyncGenerator[str, None]:
        try:
            logger.info(f"Streaming request for user {current_user.id}: {request_data.query[:50]}...")

            # Send start event
            yield f"event: start\ndata: {json.dumps({'message': 'Starting generation...', 'provider': request_data.provider})}\n\n"
            ...
            async for event in orchestrator.stream_process(
                ...
                provider=request_data.provider or "groq"
            ):
