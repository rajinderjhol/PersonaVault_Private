...
from app.models import (
    ...
    BehaviourEvent, SemanticPattern
)
...
# Helper function for safe metric getting
def _safe_metric_get(metric, labels=None):
...
async def list_models(request: Request, user_id: int = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    ...
    try:
        stmt = select(SystemConfig).where(SystemConfig.key == "ollama_model")
            ...
            try:
                ...
                if readings["heart_rate"] > 75:
                    ...
                    await manager.broadcast(json.dumps({
                        "type": "thought_stream",
                        "agent": "Reasoner",
                        "content": "Evaluating risk factors. Confidence: 0.92."
                    }))
