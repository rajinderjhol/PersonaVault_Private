...
async def get_learning_dashboard_tab():
    """Return the learning dashboard tab HTML."""
    import os
    from fastapi.responses import HTMLResponse
    
    templates_dir = os.path.join(os.path.dirname(__file__), "templates")
    html_path = os.path.join(templates_dir, "learning_dashboard.html")
    ...
    if os.path.exists(html_path):
        ...
        return HTMLResponse(content="""
        <div id="learning-dashboard-tab" style="padding: 20px;">
            <h2 style="color: var(--accent);">📊 Learning Dashboard</h2>
            <div class="card">
                <p style="color: #94a3b8;">Learning dashboard is being set up. Check back soon!</p>
            </div>
        </div>
        """)
...
async def get_tab(tab_id: str, user_id: int = Depends(require_admin)):
    ...
    if tab_path.exists():
        with open(tab_path, "r") as f:
            return HTMLResponse(content=f.read())
    return HTMLResponse(content="<div style='padding:20px;'>Tab not found</div>")
...
@router.get("/models/provider-stats/{provider}")
